    ...
    public function call_center_voice(Request $request)
    {
        ...
        if($is_active == 1) {
            ...
            if ($current_call == NULL) {
                ...
                $current_call->session_id = $session_id;
                $current_call->phone = $phone_number;
                $current_call->call_type = $direction;
                $current_call->active = $is_active;
                $current_call->agent_phone = $agent_called;
                $current_call->call_menu_selected = 1;
                $current_call->save();   // save the current call and all its attributes
            }
